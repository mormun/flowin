...
export function DashboardSidebar({ onClose }: { onClose?: () => void }) {
  ...
  // Construir el menú de navegación según el rol del usuario
  // y marcar el ítem activo comparando con la ruta actual
  const menuItems = useMemo<MenuItem[]>(() => {
    ...
    return items.map((item) => ({ ...item, active: pathname === item.href }))
  }, [pathname, role])
  ...
  return (
    ...
    >
      {/* Cabecera del sidebar: logo + botón cerrar (solo móvil) */}
      <div
        style={{
          padding: "0.75rem 0.5rem 1.5rem", marginBottom: "0.5rem",
      ...
      >
        ...
        <button
          ...
          style={{ width: "28px", height: "28px", borderRadius: "var(--radius-md)", color: "var(--color-text-muted)", alignItems: "center", justifyContent: "center", background: "none", border: "none", cursor: "pointer" }}
      ...
      {/* Ítems de navegación — ítem activo resaltado con fondo y punto indicador */}
        ...
        {menuItems.map((item) => (
          <Link
            ...
            style={{
              ...
              color: item.active ? "var(--color-primary-dark)" : "#3d3d3d",
          ...
          >
            {/* Punto indicador de ítem activo */}
            {item.active && (
