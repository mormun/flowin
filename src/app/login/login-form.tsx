...
export default function LoginForm() {
  ...
  // Mostrar errores de OAuth que llegan por query param (?error=)
  // y limpiar la URL tras mostrarlos
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validación de campos antes de llamar a la API
    if (!user && !password) { toast.warning("Campos vacíos", { description: "Introduce email y contraseña" }); return }
    if (!user) { toast.warning("Falta el email", { description: "Introduce tu correo electrónico" }); return }
    if (!password) { toast.warning("Falta la contraseña", { description: "Introduce tu contraseña" }); return }
  ...
  // Login con Google OAuth — redirige al proveedor directamente
  ...
  return (
    <div className="flex h-screen flex-col md:flex-row">
      ...
      >
        <div className="w-full max-w-sm">
          ...
          <div className="mb-16 flex justify-center">
            <Image src="/logo_render.png" alt="Flow-in" width={150} height={94} priority className="h-auto w-auto" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium" style={{ color: "var(--color-text)" }}>Contraseña</label>
              <input
                type="password"
                placeholder="••••••••"
                value={password}
            ...
            <button
              ...
              onMouseEnter={(e) => { if (!loading && !googleLoading) (e.currentTarget as HTMLElement).style.backgroundColor = "var(--color-primary-hover)" }}
              onMouseLeave={(e) => { if (!loading && !googleLoading) (e.currentTarget as HTMLElement).style.backgroundColor = "var(--color-primary)" }}
            >
          ...
          {/* Separador */}
          <div className="my-8 flex items-center gap-3" style={{ color: "var(--color-text-faint)" }}>
          ...
          <button
            ...
            onMouseEnter={(e) => { if (!loading && !googleLoading) (e.currentTarget as HTMLElement).style.backgroundColor = "var(--color-surface-offset)" }}
            onMouseLeave={(e) => { if (!loading && !googleLoading) (e.currentTarget as HTMLElement).style.backgroundColor = "var(--color-surface)" }}
          >
            {/* SVG oficial de Google con colores de marca */}
      ...
      {/* Panel derecho: decorativo, solo visible en pantallas medianas o más */}
      <div
        className="hidden flex-1 md:flex md:items-center md:justify-center"
        style={{ backgroundColor: "var(--color-bg)" }}
      >
        <div className="flex flex-col items-center gap-4 select-none" style={{ color: "var(--color-text-faint)", opacity: 0.4 }}>
