...
import {
  ...
  Clock, CheckCircle, XCircle, AlertCircle, TrendingUp, UserCheck,
} from "lucide-react"
...
const SHORTCUTS: Record<string, { label: string; icon: React.ReactNode; href: string; description: string }[]> = {
  user: [
    { label: "Mis tickets",    icon: <Ticket size={28} />,     href: "/dashboard/tickets/my",  description: "Ver todos tus tickets" },
...
// ── Icono de estadística según título ────────────────────────
const STAT_ICONS: Record<string, React.ReactNode> = {
  "Tickets creados":       <Ticket size={20} />,
  "Tickets abiertos":      <AlertCircle size={20} />,
  "Tickets en proceso":    <Clock size={20} />,
  "Tickets solucionados":  <CheckCircle size={20} />,
  "Tickets cerrados":      <XCircle size={20} />,
  "Sin asignar":           <Inbox size={20} />,
  "Mis tickets":           <Ticket size={20} />,
  "Cerrados este mes":     <CheckCircle size={20} />,
  "Usuarios activos":      <UserCheck size={20} />,
  "Categorías activas":    <Tag size={20} />,
  "Tickets totales":       <TrendingUp size={20} />,
}

const STAT_COLORS: Record<string, { bg: string; color: string; border: string }> = {
  "Tickets creados":       { bg: "#edf8f8", color: "#1e9a93", border: "#c5eeec" },
  "Tickets abiertos":      { bg: "#fef5e7", color: "#b45309", border: "#fde8c0" },
  "Tickets en proceso":    { bg: "#eff6ff", color: "#2563eb", border: "#bfdbfe" },
  "Tickets solucionados":  { bg: "#edf7e6", color: "#437a22", border: "#c3e6b5" },
  "Tickets cerrados":      { bg: "#f3f4f6", color: "#6b7280", border: "#e5e7eb" },
  "Sin asignar":           { bg: "#fef2f2", color: "#b91c1c", border: "#fecaca" },
  "Mis tickets":           { bg: "#edf8f8", color: "#1e9a93", border: "#c5eeec" },
  "Cerrados este mes":     { bg: "#edf7e6", color: "#437a22", border: "#c3e6b5" },
  "Usuarios activos":      { bg: "#edf8f8", color: "#1e9a93", border: "#c5eeec" },
  "Categorías activas":    { bg: "#eff6ff", color: "#2563eb", border: "#bfdbfe" },
  "Tickets totales":       { bg: "#fef5e7", color: "#b45309", border: "#fde8c0" },
}

// ── Componente tarjeta de estadística ────────────────────────
function StatsCard({ title, value }: { title: string; value: number }) {
  const style = STAT_COLORS[title] ?? { bg: "var(--color-surface-offset)", color: "var(--color-text-muted)", border: "var(--color-border)" }
  const icon = STAT_ICONS[title] ?? <TrendingUp size={20} />
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: "48px", height: "48px", backgroundColor: style.bg, color: style.color, border: `1px solid ${style.border}` }}
      >
        {icon}
      </div>
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--color-text-faint)", marginBottom: "0.2rem" }}>
          {title}
        </p>
        <p className="text-3xl font-bold tabular" style={{ color: "var(--color-text)", lineHeight: 1.1 }}>
          {value}
        </p>
      </div>
    </div>
  ...
  return (
    <>
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className={`${height} animate-pulse rounded-xl`}
  ...
  return (
    <div>
      <p
        ...
        style={{ color: "var(--color-text-faint)", marginBottom: "0.875rem", paddingLeft: "0.25rem" }}
      >
        {label}
      </p>
      <div
