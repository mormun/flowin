...
// Color del indicador visual por nombre de estado
...
// Estilo para inputs de edición inline en tabla
...
// Botón icono cuadrado reutilizable
const IconBtn = ({ icon, onClick, title }: { icon: React.ReactNode; onClick: () => void; title?: string }) => (
  <button
    onClick={onClick}
    title={title}
    aria-label={title}
...
export default function StatusPage() {
  ...
  const fetchStatuses = async () => {
    try {
      ...
      if (!res.ok) { toast.error(data?.error ?? "Error cargando estados"); return }
      setStatuses(data)
    } catch {
  ...
  // Guardar edición de nombre y descripción de un estado
  const handleEdit = async (id: number) => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { toast.error(data?.error ?? "Error al actualizar estado"); return }
  ...
  return (
    <div style={{ width: "100%", padding: "2rem 3rem 2rem 2.5rem", marginTop: "1.5rem" }}>
      <div style={{ marginBottom: "1.25rem" }}>
        <h2 className="text-2xl font-bold" style={{ color: "var(--color-text)" }}>Estados</h2>
        <p className="mt-1 text-sm" style={{ color: "var(--color-text-muted)" }}>
          Puedes editar el nombre y la descripción. No es posible crear ni eliminar estados.
        </p>
      </div>

      {/* Aviso: modificar nombres de estados puede romper la lógica interna */}
      <div
      ...
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-warning)" strokeWidth="2" className="mt-0.5 shrink-0">
          <path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" />
          <line x1="12" y1="9" x2="12" y2="13" />
          <line x1="12" y1="17" x2="12.01" y2="17" />
        </svg>
        <p className="text-sm" style={{ color: "var(--color-warning)" }}>
          Modificar el nombre de un estado puede afectar a la lógica interna de la aplicación. Hazlo solo si es estrictamente necesario.
        </p>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {statuses.map((s, idx) => (
                <tr
                  key={s.id}
                  style={{ borderBottom: idx < statuses.length - 1 ? "1px solid var(--color-divider)" : "none" }}
                >
                  <td
                  ...
                  {editingId === s.id ? (
                    <>
                      ...
                      <td className="py-2 pr-2">
                        <input style={inlineInputStyle} value={editDescription} onChange={(e) => setEditDescription(e.target.value)} placeholder="Descripción" />
                      </td>
                    ...
                    // Modo lectura: punto de color + nombre + descripción
                    <>
                      <td className="py-3.5 pr-4">
                        <div className="flex items-center gap-2.5">
                          <span
                            className="h-2.5 w-2.5 shrink-0 rounded-full"
                            style={{ backgroundColor: STATUS_DOT[s.name] ?? "var(--color-text-faint)" }}
                          />
                          <span className="text-base font-medium" style={{ color: "var(--color-text)" }}>{s.name}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-3.5 pr-6">
                        <div className="flex justify-end">
                          <IconBtn
                            icon={<Pencil size={16} />}
                            onClick={() => { setEditingId(s.id); setEditName(s.name); setEditDescription(s.description ?? "") }}
                            title="Editar"
                          />
                        </div>
                      </td>
